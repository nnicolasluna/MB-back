...
const DEFAULT_MONITORING_WATER_STYLE = 'default_monitoring_burn';

@Injectable()
export class MonitoringBurnService {
	private logger = new Logger(MonitoringBurnService.name);
	...
	async create(dto: FormMonitoringBurnDto) {
		const existMapData = await this.db.monitoringBurn.findFirst({
			where: {
				name: dto.name,
		...
		const data = new MonitoringBurnEntity(updateData);

		data.layer = generateCleanName(dto.name, 'burn_');
		...
		const createdData = await this.db.monitoringBurn.create({ data });

		if (zipFile) await this.uploadMap(zipFile, data.layer);

		return new MonitoringBurnEntity(createdData);
	}

	async findAll(filter: MonitoringBurnFilter) {
		this.logger.log(`Finding all burn monitoring, with filter: ${filter}`);

		const { where, pagination } = filter;

		const [data, total] = await this.db.$transaction([
			this.db.monitoringBurn.findMany({
				where,
				...pagination,
				include: { createdBy: { select: { name: true, firstSurname: true } } },
			}),
			this.db.monitoringBurn.count({ where }),
		]);

		return new ListMonitoringBurnDto(data, total);
	}

	async findOne(id: number) {
		this.logger.log(`Finding map with id: ${id}`);
		const data = await this.db.monitoringBurn.findFirst({ where: { id } });

		if (!data) throw new NotFoundException();

		return new MonitoringBurnEntity(data);
	}

	async getStadistics(q: StadisticsFilter) {
		this.logger.log(`Getting stadistics with filter: ${JSON.stringify(q)}`);

		let monitoring: any = await this.db.monitoringBurn.findFirst({
			where: {
				uuid: q.uuid,
		...
		if (!monitoring) {
			monitoring = await this.db.monitoringBurn.findFirst({
				where: {
					coverageState: 'Activo',
					state: true,
					NOT: {
	...
	async getStadisticsFromLayer(layer: string) {
	...
	async update(id: number, dto: FormMonitoringBurnDto) {
		this.logger.log(`Updating map with id: ${id}`);

		const { zipFile, ...updateData } = dto;

		const res = await this.db.monitoringBurn.update({
			where: { id },
			data: updateData,
		});
		...
		return new MonitoringBurnEntity(res);
	}

	async remove(id: number, body: AuditDeleteFields) {
		const data = await this.db.monitoringBurn.findUnique({ where: { id } });

		if (!data) throw new NotFoundException();

		await this.geoserverService.deleteLayer(data.layer);

		await this.db.monitoringBurn.update({
			where: { id },
			data: {
				state: false,
				...body,
	...
	async updateStatus(code: string, dto: AuditFields & { status: string }) {
		...
		const data = await this.db.monitoringBurn.findUnique({ where: { uuid: code } });
	...
	async exportShpToZip(uuid: string) {
		try {
			const map = await this.db.monitoringBurn.findUnique({
				where: { uuid },
				select: { layer: true },
			});
