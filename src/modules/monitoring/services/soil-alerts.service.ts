...
const DEFAULT_MONITORING_WATER_STYLE = 'default_soil_alert';

@Injectable()
export class SoilAlertsService {
	private logger = new Logger(SoilAlertsService.name);
	...
	async create(dto: FormSoilAlertDto) {
		const existMapData = await this.db.soilAlert.findFirst({
			where: {
				name: dto.name,
		...
		const data = new SoilAlertEntity(updateData);

		data.layer = generateCleanName(dto.name, 'burn_h_');
		...
		const createdData = await this.db.soilAlert.create({ data });

		if (zipFile) await this.uploadMap(zipFile, data.layer);

		return new SoilAlertEntity(createdData);
	}

	async findAll(filter: SoilAlertFilter) {
		this.logger.log(`Finding all burn historic, with filter: ${filter}`);

		const { where, pagination } = filter;

		const [data, total] = await this.db.$transaction([
			this.db.soilAlert.findMany({
				where,
				...pagination,
				include: { createdBy: { select: { name: true, firstSurname: true } } },
			}),
			this.db.soilAlert.count({ where }),
		]);

		return new ListSoilAlertDto(data, total);
	}

	async findOne(id: number) {
		this.logger.log(`Finding map with id: ${id}`);
		const data = await this.db.soilAlert.findFirst({ where: { id } });

		if (!data) throw new NotFoundException();

		return new SoilAlertEntity(data);
	}

	async update(id: number, dto: FormSoilAlertDto) {
		this.logger.log(`Updating map with id: ${id}`);

		const { zipFile, ...updateData } = dto;

		const res = await this.db.soilAlert.update({
			where: { id },
			data: updateData,
		});
		...
		return new SoilAlertEntity(res);
	}

	async remove(id: number, body: AuditDeleteFields) {
		const data = await this.db.soilAlert.findUnique({ where: { id } });

		if (!data) throw new NotFoundException();

		await this.geoserverService.deleteLayer(data.layer);

		await this.db.soilAlert.update({
			where: { id },
			data: {
				state: false,
				...body,
		...
		const job = await this.queue.add(
			{
				...
				name: file.filename,
				table,
				type: MonitoringType.SOIL_ALERTS,
				skipProcess: true,
			} as ProcessMapJobDto,
	...
	async updateStatus(code: string, dto: AuditFields & { status: string }) {
		...
		const data = await this.db.soilAlert.findUnique({ where: { uuid: code } });
	...
	async exportShpToZip(uuid: string) {
		try {
			const map = await this.db.soilAlert.findUnique({
				where: { uuid },
				select: { layer: true },
			});
	...
	async getStadistics(q: StadisticsFilter) {
		...
		let monitoring: any = await this.db.soilAlert.findFirst({
			where: {
				uuid: q.uuid,
		...
		if (!monitoring) {
			monitoring = await this.db.soilAlert.findFirst({
				where: {
					coverageState: 'Activo',
					state: true,
					NOT: {
