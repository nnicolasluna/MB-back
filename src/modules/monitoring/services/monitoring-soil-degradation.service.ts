...
import { FormMonitoringSoilDegradationDto } from '../dto/monitoring-soil-degradation/form-monitoring-soil-degradation.dto';
import { generateCleanName, SelectQueryBuilder } from '@shared/utils';
import { MonitoringSoilDegradationFilter } from '../dto/monitoring-soil-degradation/monitoring-soil-degradation.filter';
import { ListMonitoringSoilDegradationDto } from '../dto/monitoring-soil-degradation/list-monitoring-soil-degradation.dto';
...
import { MonitoringSoilDegradationEntity } from '../entities/monitoring-soil-degradation.entity';
import { cartographicSchema } from '@modules/map-data/lib/constants';
import { ProcessMapJobDto, MonitoringType } from '../dto/upload-map-job.dto';
import { StadisticsFilter } from '../dto/stadistics.filter';

const DEFAULT_MONITORING_STYLE = 'default_monitoring_soil';

@Injectable()
export class MonitoringSoilDegradationService {
	private logger = new Logger(MonitoringSoilDegradationService.name);
	...
	async create(dto: FormMonitoringSoilDegradationDto) {
		const existMapData = await this.db.monitoringSoilDegradation.findFirst({
			where: {
				name: dto.name,
		...
		const data = new MonitoringSoilDegradationEntity(updateData);

		data.layer = generateCleanName(dto.name, 'soil_');
		data.styleName = DEFAULT_MONITORING_STYLE;
		data.coverageState = 'Activo';

		this.logger.log(`Creating map ${data.name}, layer ${data.layer}`);

		const createdData = await this.db.monitoringSoilDegradation.create({ data });

		if (zipFile) await this.uploadMap(zipFile, data.layer);

		return new MonitoringSoilDegradationEntity(createdData);
	}

	async findAll(filter: MonitoringSoilDegradationFilter) {
		this.logger.log(`Finding all soil-degradation monitoring, with filter: ${filter}`);

		const { where, pagination } = filter;

		const [data, total] = await this.db.$transaction([
			this.db.monitoringSoilDegradation.findMany({
				where,
				...pagination,
				include: { createdBy: { select: { name: true, firstSurname: true } } },
			}),
			this.db.monitoringSoilDegradation.count({ where }),
		]);

		return new ListMonitoringSoilDegradationDto(data, total);
	}

	async findOne(id: number) {
		this.logger.log(`Finding map with id: ${id}`);
		const data = await this.db.monitoringSoilDegradation.findFirst({ where: { id } });

		if (!data) throw new NotFoundException();

		return new MonitoringSoilDegradationEntity(data);
	}

	async getStadistics(q: StadisticsFilter) {
		this.logger.log(`Getting stadistics with filter: ${JSON.stringify(q)}`);

		let monitoring: any = await this.db.monitoringSoilDegradation.findFirst({
			where: {
				uuid: q.uuid,
		...
		if (!monitoring) {
			monitoring = await this.db.monitoringSoilDegradation.findFirst({
				where: {
					coverageState: 'Activo',
					state: true,
					NOT: {
	...
	async update(id: number, dto: FormMonitoringSoilDegradationDto) {
		this.logger.log(`Updating map with id: ${id}`);

		const { zipFile, ...updateData } = dto;

		const res = await this.db.monitoringSoilDegradation.update({
			where: { id },
			data: updateData,
		});
		...
		return new MonitoringSoilDegradationEntity(res);
	}

	async remove(id: number, body: AuditDeleteFields) {
		const data = await this.db.monitoringSoilDegradation.findUnique({ where: { id } });

		if (!data) throw new NotFoundException();

		await this.geoserverService.deleteLayer(data.layer);

		await this.db.monitoringSoilDegradation.update({
			where: { id },
			data: {
				state: false,
				...body,
	...
	async updateStatus(code: string, dto: AuditFields & { status: string }) {
		...
		const data = await this.db.monitoringSoilDegradation.findUnique({ where: { uuid: code } });
	...
	async exportShpToZip(uuid: string) {
		try {
			const map = await this.db.monitoringSoilDegradation.findUnique({
				where: { uuid },
				select: { layer: true },
			});
	...
	// TODO: add functions for monitoringsoil degradation
