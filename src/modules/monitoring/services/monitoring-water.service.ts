...
import { ProcessMapJobDto } from '../dto/upload-map-job.dto';
...
const DEFAULT_MONITORING_WATER_STYLE = 'default_monitoring_water';

@Injectable()
export class MonitoringWaterService {
	private logger = new Logger(MonitoringWaterService.name);
	...
	async create(dto: FormMonitoringWaterDto) {
		const existMapData = await this.db.monitoringWater.findFirst({
			where: {
				name: dto.name,
		...
		const data = new MonitoringWaterEntity(updateData);

		data.layer = generateCleanName(dto.name, 'water_');
		...
		const createdData = await this.db.monitoringWater.create({ data });

		if (zipFile) await this.uploadMap(zipFile, data.layer);

		return new MonitoringWaterEntity(createdData);
	}

	async findAll(filter: MonitoringWaterFilter) {
		this.logger.log(`Finding all water monitoring, with filter: ${filter}`);

		const { where, pagination } = filter;

		const [data, total] = await this.db.$transaction([
			this.db.monitoringWater.findMany({
				where,
				...pagination,
				include: { createdBy: { select: { name: true, firstSurname: true } } },
			}),
			this.db.monitoringWater.count({ where }),
		]);

		return new ListMonitoringWaterDto(data, total);
	}

	async findOne(id: number) {
		this.logger.log(`Finding map with id: ${id}`);
		const data = await this.db.monitoringWater.findFirst({ where: { id } });

		if (!data) throw new NotFoundException();

		return new MonitoringWaterEntity(data);
	}

	async getStadistics(q: StadisticsFilter) {
		this.logger.log(`Getting stadistics with filter: ${JSON.stringify(q)}`);

		let monitoring: any = await this.db.monitoringWater.findFirst({
			where: {
				uuid: q.uuid,
		...
		if (!monitoring) {
			monitoring = await this.db.monitoringWater.findFirst({
				where: {
					coverageState: 'Activo',
					state: true,
					NOT: {
	...
	async getResumeYears() {
		const monitorings: any[] = await this.db.monitoringWater.findMany({
			where: {
				coverageState: 'Activo',
			},
			orderBy: {
		...
		const resumeYears: any = {};

		for (const monitoring of monitorings) {
			const year = dayjs(monitoring.coverageDate).utc().format('DD/MM/YYYY');

			const data = await this.getStadisticsFromLayer(monitoring.layer);
			...
			data.forEach((x: any) => {
				if (x.mun != null) resumeYears[year] += x.sup;
			});
		}
	...
	async getSymDifference(monitoring1uuid: string, monitoring2uuid: string) {
		const res = await this.cacheManager.get(`${monitoring1uuid}-${monitoring2uuid}`);

		this.logger.log(`Getting sym difference from cache: ${monitoring1uuid}-${monitoring2uuid}`);
		this.logger.debug(res);

		if (res) return res;

		const mon1 = await this.db.monitoringWater.findUnique({
			where: { uuid: monitoring1uuid },
		});
		const mon2 = await this.db.monitoringWater.findUnique({
			where: { uuid: monitoring2uuid },
		});
	...
	t1.tcos,
	t1.ramsar,
	ST_Area(ST_Union(t1.geom), true) / 10000 as sup_1,
	ST_Area(ST_Union(t2.geom), true) / 10000 as sup_2,
	ST_Area(ST_SymDifference(ST_Union(t1.geom), ST_Union(t2.geom)), true) / 10000 as difference
from
	cartographic.${mon1.layer} t1, cartographic.${mon2.layer} t2
where
	t1.mun = t2.mun or t1.aps = t2.aps or t1.tcos = t2.tcos or t1.ramsar = t2.ramsar
group by
	t1.mun, t1.prov, t1.aps, t1.tcos, t1.ramsar`;

		const result = await this.db.$queryRawUnsafe(q);
		await this.cacheManager.set(`${monitoring1uuid}-${monitoring2uuid}`, JSON.stringify(result), 0);
		return result;
	}

	async update(id: number, dto: FormMonitoringWaterDto) {
		this.logger.log(`Updating map with id: ${id}`);

		const { zipFile, ...updateData } = dto;

		const res = await this.db.monitoringWater.update({
			where: { id },
			data: updateData,
		});
		...
		return new MonitoringWaterEntity(res);
	}

	async remove(id: number, body: AuditDeleteFields) {
		const data = await this.db.monitoringWater.findUnique({ where: { id } });

		if (!data) throw new NotFoundException();

		await this.geoserverService.deleteLayer(data.layer);

		await this.db.monitoringWater.update({
			where: { id },
			data: {
				state: false,
				...body,
	...
	async updateStatus(code: string, dto: AuditFields & { status: string }) {
		...
		const data = await this.db.monitoringWater.findUnique({ where: { uuid: code } });
	...
	async exportShpToZip(uuid: string) {
		try {
			const map = await this.db.monitoringWater.findUnique({
				where: { uuid },
				select: { layer: true },
			});
