...
const DEFAULT_MONITORING_STYLE = 'default_monitoring_use';

@Injectable()
export class MonitoringUseLandService {
	private logger = new Logger(MonitoringUseLandService.name);
	...
	async create(dto: FormMonitoringUseLandDto) {
		const existMapData = await this.db.monitoringUseLand.findFirst({
			where: {
				name: dto.name,
		...
		const data = new MonitoringUseLandEntity(updateData);

		data.layer = generateCleanName(dto.name, 'use_');
		data.styleName = DEFAULT_MONITORING_STYLE;
		data.coverageState = 'Activo';

		this.logger.log(`Creating map ${data.name}, layer ${data.layer}`);

		const createdData = await this.db.monitoringUseLand.create({ data });

		if (zipFile) await this.uploadMap(zipFile, data.layer);

		return new MonitoringUseLandEntity(createdData);
	}

	async findAll(filter: MonitoringUseLandFilter) {
		this.logger.log(`Finding all use-land monitoring, with filter: ${filter}`);

		const { where, pagination } = filter;

		const [data, total] = await this.db.$transaction([
			this.db.monitoringUseLand.findMany({
				where,
				...pagination,
				include: { createdBy: { select: { name: true, firstSurname: true } } },
			}),
			this.db.monitoringUseLand.count({ where }),
		]);

		return new ListMonitoringUseLandDto(data, total);
	}

	async findOne(id: number) {
		this.logger.log(`Finding map with id: ${id}`);
		const data = await this.db.monitoringUseLand.findFirst({ where: { id } });

		if (!data) throw new NotFoundException();

		return new MonitoringUseLandEntity(data);
	}

	async getStadistics(q: StadisticsFilter) {
		this.logger.log(`Getting stadistics with filter: ${JSON.stringify(q)}`);

		let monitoring: any = await this.db.monitoringUseLand.findFirst({
			where: {
				uuid: q.uuid,
		...
		if (!monitoring) {
			monitoring = await this.db.monitoringUseLand.findFirst({
				where: {
					coverageState: 'Activo',
					state: true,
					NOT: {
	...
	async update(id: number, dto: FormMonitoringUseLandDto) {
		this.logger.log(`Updating map with id: ${id}`);

		const { zipFile, ...updateData } = dto;

		const res = await this.db.monitoringUseLand.update({
			where: { id },
			data: updateData,
		});
		...
		return new MonitoringUseLandEntity(res);
	}

	async remove(id: number, body: AuditDeleteFields) {
		const data = await this.db.monitoringUseLand.findUnique({ where: { id } });

		if (!data) throw new NotFoundException();

		await this.geoserverService.deleteLayer(data.layer);

		await this.db.monitoringUseLand.update({
			where: { id },
			data: {
				state: false,
				...body,
	...
	async updateStatus(code: string, dto: AuditFields & { status: string }) {
		...
		const data = await this.db.monitoringUseLand.findUnique({ where: { uuid: code } });
	...
	async exportShpToZip(uuid: string) {
		try {
			const map = await this.db.monitoringUseLand.findUnique({
				where: { uuid },
				select: { layer: true },
			});
	...
	// TODO: add functions for monitoringuse-land
