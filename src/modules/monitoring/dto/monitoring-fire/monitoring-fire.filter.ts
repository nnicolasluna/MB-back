import { MonitoringFire, Prisma } from '@prisma/client';
import { PaginableFilter } from '@shared/dto';
import { createNestedObject } from '@shared/utils';

const searchFields: (keyof MonitoringFire)[] = ['id', 'name', 'description'];

export class MonitoringFireFilter extends PaginableFilter {
	coverageState?: string;

	get where() {
		const where: Prisma.MonitoringFireWhereInput = {
			state: true,
			deletedDate: null,
			idDeletedBy: null,
		};
