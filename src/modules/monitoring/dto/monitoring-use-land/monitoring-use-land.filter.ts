import { MonitoringUseLand, Prisma } from '@prisma/client';
import { PaginableFilter } from '@shared/dto';
import { createNestedObject } from '@shared/utils';

const searchFields: (keyof MonitoringUseLand)[] = ['id', 'name', 'description'];

export class MonitoringUseLandFilter extends PaginableFilter {
	coverageState?: string;

	get where() {
		const where: Prisma.MonitoringUseLandWhereInput = {
			state: true,
			deletedDate: null,
			idDeletedBy: null,
		};
