...
@Controller('monitoring-fire')
@ApiBearerAuth()
@ApiTags('Monitoring Fire')
export class MonitoringFireController {
	constructor(private readonly service: MonitoringFireService) {}

	@Get()
	@ApiOperation({ summary: 'Get list of monitoring fire' })
	@ApiOkResponse({ type: ListMonitoringFireDto })
	@CanRead(RESOURCE_NAME)
	async findAll(@Query() filter: MonitoringFireFilter) {
	...
	@ApiOkResponse({ type: MonitoringFireEntity })
	@CanRead(RESOURCE_NAME)
	...
	create(@UploadedFile() file: Express.Multer.File, @Body() dto: FormMonitoringFireDto) {
		dto.zipFile = file;
	...
	update(@UploadedFile() file: Express.Multer.File, @Param('id') id: string, @Body() dto: FormMonitoringFireDto) {
		dto.zipFile = file;
